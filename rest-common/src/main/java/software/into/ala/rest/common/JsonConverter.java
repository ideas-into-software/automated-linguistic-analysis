...
public class JsonConverter<T> implements MessageBodyReader<T>, MessageBodyWriter<T> {
	private final Converter converter = Converters.newConverterBuilder().rule(JsonValue.class, this::toJsonValue)
			.rule(this::toScalar).build();
	...
	private JsonValue toJsonValue(Object value, Type targetType) {
		if (value == null) {
			...
			return Json.createValue(((Enum<?>) value).name().toString());
